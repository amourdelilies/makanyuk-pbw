...
import { FaClock, FaFireAlt, FaRegBookmark, FaBookmark, FaFilter, FaStar, FaHeart, FaRegHeart, FaShareAlt, FaChevronLeft, FaCheckCircle } from 'react-icons/fa';
...
export default function Recipes() {
  ...
  const categories = ['Semua', 'Ayam', 'Kambing', 'Sapi', 'Ikan', 'Sayur'];
  ...
  useEffect(() => {
    const fetchAllRecipes = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch('https://makan-yuk.vercel.app/api/recipe/');
        if (!response.ok) throw new Error('Gagal memuat database dari server MakanYuk.');
        
        const resData = await response.json();
        let finalData = Array.isArray(resData) ? resData : resData.data || resData.recipes || [];
        setAllRecipes(finalData);
        setFilteredRecipes(finalData);
      } catch (err) {
  ...
  useEffect(() => {
    if (activeTab === 'Semua') {
      ...
      const filtered = allRecipes.filter(recipe => {
        ...
        const cleanCategory = activeTab.toLowerCase();
        if (cleanCategory === 'sapi') return cleanTitle.includes('sapi') || cleanTitle.includes('daging') || cleanTitle.includes('baso') || cleanTitle.includes('bakso');
        if (cleanCategory === 'sayur') return cleanTitle.includes('sayur') || cleanTitle.includes('oseng') || cleanTitle.includes('capcay') || cleanTitle.includes('sup') || cleanTitle.includes('salad');
        return cleanTitle.includes(cleanCategory);
      });
  ...
  const getStrictIndonesianImage = (title) => {
    ...
    if (cleanTitle.includes('goreng') && cleanTitle.includes('ayam')) {
      return ['https://images.unsplash.com/photo-1626082927389-6cd097cdc6ec?q=80&w=500&auto=format&fit=crop', 'https://images.unsplash.com/photo-1562967914-608f82629710?q=80&w=500&auto=format&fit=crop', 'https://images.unsplash.com/photo-1606755962773-d324e0a13086?q=80&w=500&auto=format&fit=crop', 'https://images.unsplash.com/photo-1569058242253-92a9c755a0ec?q=80&w=500&auto=format&fit=crop'][salt % 4];
    }
    ...
    if (cleanTitle.includes('capcay') || cleanTitle.includes('oseng') || cleanTitle.includes('tumis')) return 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?q=80&w=500&auto=format&fit=crop';
    ...
    if (cleanTitle.includes('kambing')) return 'https://images.unsplash.com/photo-1608897013039-887f21d8c804?q=80&w=500&auto=format&fit=crop';
    if (cleanTitle.includes('sapi') || cleanTitle.includes('daging')) return 'https://images.unsplash.com/photo-1603048588665-791ca8aea617?q=80&w=500&auto=format&fit=crop';
    if (cleanTitle.includes('ikan') || cleanTitle.includes('tongkol')) return 'https://images.unsplash.com/photo-1519708227418-c8fd9a32b7a2?q=80&w=500&auto=format&fit=crop';

    return 'https://images.unsplash.com/photo-1626700051175-6518c4793f4f?q=80&w=500&auto=format&fit=crop';
  };
  ...
  if (view === 'list') {
    return (
      <div className="space-y-8 min-h-[70vh] px-2 sm:px-0">
        ...
        {/* PERBAIKAN DI SINI: Ditambahkan 'pb-4' agar efek bayangan / shadow tombol tidak terpotong saat di-scroll */}
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs sm:text-sm font-semibold transition-all duration-300 ${
                activeTab === cat ? 'bg-green-700 text-white shadow-lg shadow-green-700/30 scale-105' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
              }`}
            >
              {cat}
            </button>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-red-50 rounded-2xl border border-red-100 text-red-600 text-sm max-w-xl mx-auto px-4 shadow-sm">⚠️ Gagal memuat data: {error}</div>
